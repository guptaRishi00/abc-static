          ...
          {cards &&
            cards.map((card: any, index: number) => {
              ...
              return (
                <div
                  key={card.id || index}
                  className={`relative rounded-3xl w-full lg:w-1/3 max-w-md mx-auto lg:mx-0 flex flex-col overflow-hidden border ${isCenter
                      ? "bg-[#1E1E1E] text-white border-[#1E1E1E] z-20"
                      : "bg-white text-[#111] border-gray-200 z-10 mt-0 lg:mt-6 mb-0 lg:mb-6"
                    }`}
                >
                  {isCenter && (
                    <div className="absolute top-5 right-5 z-30">
                      <span className="px-3 py-1 text-[10px] font-bold uppercase tracking-widest bg-white text-[#1E1E1E] rounded-full">
                        Popular
                      </span>
                    </div>
                  )}
                  ...
                  <div className={`relative px-7 pt-8 pb-6 text-center border-b z-10 ${isCenter ? 'border-white/10' : 'border-gray-100'}`}>
                    ...
                    <div className="flex flex-col items-center justify-center">
                      ...
                      {card.days && (
                        <span className={`text-sm mt-2 font-light ${isCenter ? 'text-gray-400' : 'text-[#888]'}`}>
                          {card.days}
                        </span>
                      )}
                    </div>
                    ...
                    {card.lists &&
                      card.lists.map((list: any, listIndex: number) => (
                        <div className="flex items-start gap-4" key={list.id || listIndex}>
                          <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${isCenter ? 'bg-white/10 text-white' : 'bg-gray-100 text-[#1E1E1E]'}`}>
                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                              <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                            </svg>
                          </div>
                          <p className={`text-sm lg:text-base leading-relaxed font-light ${isCenter ? 'text-gray-300' : 'text-[#555]'}`}>
                            {list.text}
                          </p>
                        </div>
                      ))}
                  ...
                  <div className={`px-7 pb-8 relative z-10 ${isCenter ? 'bg-black/20' : 'bg-transparent'}`}>
                    <Link
                      ...
                      className={`group/btn block w-full text-center font-medium text-sm lg:text-base py-4 rounded-xl transition-colors duration-300 border ${isCenter
                          ? "bg-white border-white text-[#1E1E1E] hover:bg-gray-100 hover:border-gray-100"
