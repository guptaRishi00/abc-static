  ...
  return (
    <div className="py-16 px-6 gap-12 lg:px-0 lg:gap-20 lg:py-28 w-full flex flex-col items-center justify-center bg-[#FAFAFA]">
      ...
      <div className="flex flex-col items-center gap-5 text-center">
        ...
        <div className="w-24 h-1 bg-[#1E1E1E] rounded-full mt-1"></div>
      </div>
      ...
      <div className="w-full max-w-7xl mx-auto flex flex-col md:flex-row items-stretch justify-center relative lg:px-8 gap-8 lg:gap-10 pl-12 lg:pl-8">
        ...
        <div className="hidden lg:flex absolute top-[40%] left-[12%] right-[12%] z-[1] items-center justify-between px-[15%]">
          {cards && cards.length > 1 && Array.from({ length: cards.length - 1 }).map((_, i) => (
            <div key={i} className="w-full h-px bg-gray-200 mx-4"></div>
          ))}
        </div>
        ...
        {cards &&
          cards.map((card: any, index: number) => {
            return (
              <div
                key={card.id || index}
                className="relative z-10 flex flex-col items-center flex-1 bg-white rounded-3xl border border-gray-200 p-8 lg:p-10 transition-colors duration-300 hover:bg-gray-50"
              >

                <div className={`absolute -left-6 -top-6 lg:-top-8 lg:left-1/2 lg:-translate-x-1/2 flex items-center justify-center w-16 h-16 lg:w-20 lg:h-20 bg-[#1E1E1E] rounded-2xl transition-colors duration-300`}>
